...
class GenreService(BaseElasticService):
    ...
    async def get_by_id(self, genre_id: str) -> Genre | None:
        key = self._redis.build_redis_key(self.__key_prefix, genre_id)
        genre = await self._redis.get_one_model(key, Genre)
        if not genre:
            genre = await self.__get_genre_from_elastic(genre_id)
            if not genre:
                return None
            await self._redis.set_one_model(key, genre, self._cache_ex)
        return genre
    ...
    async def get_genres(
        ...
        key = self._redis.build_redis_key(
            self.__key_prefix, page_number, page_size
        )
        genres = await self._redis.get_list_model(key, Genre)
        if not genres:
            ...
            if not genres:
                return None
            await self._redis.set_list_model(key, genres, self._cache_ex)
        return genres
...
def get_genre_service(
    ...
    return GenreService(redis, elastic_client, settings.cache_ex_for_genres)
